...
function createExcel(nameFile, dataArray) {
	if (nameFile == "AllLog") {
        ...
        for (const [name, array] of Object.entries(dataArray)) {
            ...
            allData.push({ Profile: name, Tag: '' });

            // Add the data rows
            array.forEach(item => {
                const [profile, tag] = item.split('\t');
                allData.push({ Profile: profile, Tag: tag });
            });

            // Add an empty row for separation (optional)
            allData.push({ Profile: '', Tag: '' });
        }
		...
		// Split the data into "Profile" and "Tag" columns
	    const data = dataArray.map(item => {
	        const [profile, tag] = item.split('\t');
	        return { Profile: profile, Tag: tag };
	    });
